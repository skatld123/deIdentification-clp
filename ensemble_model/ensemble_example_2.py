...
if __name__ == '__main__':
    ...
    subs = [
        # mmdetection
        pd.read_csv('./results/effb3_fpn_8xb4-crop896-1x_clp/wbf_df.csv'),
        pd.read_csv('./results/swin_clp/wbf_df.csv'),
    ]
    ...
    sample_sub = pd.read_csv('/root/ensemble_model/data/sample_submission.csv')
    height_list = []
    width_list = []

    for i, name in tqdm(enumerate(sample_sub.file_name)):
        img = opj('/root/dataset_clp/dataset_2044/test/images/', Path(name).stem + '.png')
