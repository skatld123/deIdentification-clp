...
for idx, lbl in enumerate(labels):
    if lbl == "real_A":
        virtual_data_tsne.append(tsne_results[idx])
    else:
        real_data_tsne.append(tsne_results[idx])

virtual_data_tsne = np.array(virtual_data_tsne)
real_data_tsne = np.array(real_data_tsne)

# t-SNE 결과 시각화
plt.figure(figsize=(8, 6))
plt.scatter(virtual_data_tsne[:, 0], virtual_data_tsne[:, 1], label="real_A")
plt.scatter(real_data_tsne[:, 0], real_data_tsne[:, 1], label="real_B")
plt.legend()
plt.title("t-SNE Visualization")
plt.xlabel("t-SNE feature 1")
plt.ylabel("t-SNE feature 2")
save_dir = '/root/deid-lp-GAN/data_analysis_plot/tsne_plot'
