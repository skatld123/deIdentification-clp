...
virtual_data_dir = "/root/deIdentification-clp/cross_analysis/Virtual_Number_Plate/virtual/all"
real_data_dir = "/root/deIdentification-clp/cross_analysis/rect_test_dataset"
...
# 실제 이미지 데이터 처리
for filename in tqdm(os.listdir(real_data_dir)):
    if filename.endswith(".jpg"):
...
# 데이터 시각화
plt.scatter(virtual_data[:, 0], virtual_data[:, 1], label="Virtual")
plt.scatter(real_data[:, 0], real_data[:, 1], label="Real")
plt.legend()
plt.title("PCA Virtual and Real Images")
plt.savefig('/root/deIdentification-clp/cross_analysis/result.jpg')
