...
def deIdentify(result_dir, data_path, crop_path, save_path) :
    ...
    # CycleGAN 비식별화
    deid_path = "/root/deIdentification-clp/result"
    if not os.path.exists(deid_path) : os.mkdir(deid_path)
    transform(dataroot=data_path, gpu_ids="0,1", num_test=len(os.listdir(data_path)),
              results_dir=deid_path)
